# 내일배움캠프 38일차 TIL  
오전에는 심화주차 발제가 있었다. 이후 새로운 팀을 만나고, 어쩌다 보니 팀장이 되었다. 그동안 팀장은 맡아본 적이 없었는데, 뭔가 더 책임감을 가져야만 할 것 같은 기분이 든다.  
코드카타 후 팀명 및 규칙을 정하고, 심화주차 강의를 수강했다.  
...
먼저 가지고 있는 로또 번호들을 검사하는데, 0의 개수와, 0이 아닌 숫자들 중 정답 번호와 일치하는 숫자의 수를 구했다. 이후 이 두 수를 통해 순위를 구하는 메서드를 만들어서 구현했다.  
```cs
using System;
